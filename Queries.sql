...
-- Вариант составного индекса из трех элементов, необходимо найти в контактах 
-- только Виталиев и только регион 798 для телефон, т.е. разобрать строки полей.

-- до индексирования

EXPLAIN SELECT name, number FROM contacts 
WHERE contacts.volume > 30   
AND (SELECT LOCATE('Виталий', contacts.name))!=0   
AND (SELECT LOCATE('978', contacts.number))!=0;
+----+-------------+----------+------------+------+---------------+------+---------+------+--------+----------+-------------+
| id | select_type | table    | partitions | type | possible_keys | key  | key_len | ref  | rows   | filtered | Extra       |
+----+-------------+----------+------------+------+---------------+------+---------+------+--------+----------+-------------+
|  1 | SIMPLE      | contacts | NULL       | ALL  | NULL          | NULL | NULL    | NULL | 496790 |    33.33 | Using where |
+----+-------------+----------+------------+------+---------------+------+---------+------+--------+----------+-------------+
1 row in set, 5 warnings (0.02 sec)

-- после добавления индекса, обработка строк уменьшилась в 2 раза.

+----+-------------+----------+------------+-------+-----------------+-----------------+---------+------+--------+----------+--------------------------+
| id | select_type | table    | partitions | type  | possible_keys   | key             | key_len | ref  | rows   | filtered | Extra                    |
+----+-------------+----------+------------+-------+-----------------+-----------------+---------+------+--------+----------+--------------------------+
|  1 | SIMPLE      | contacts | NULL       | range | vol_name_number | vol_name_number | 5       | NULL | 248395 |   100.00 | Using where; Using index |
+----+-------------+----------+------------+-------+-----------------+-----------------+---------+------+--------+----------+--------------------------+
